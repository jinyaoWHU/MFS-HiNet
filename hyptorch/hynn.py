...
class ToPoincare(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.train_x:
            ...
            return self.grad_fix(pmath.project(pmath.expmap(xp, x, c=self.c), c=self.c)), self.c
        return self.grad_fix(pmath.project(pmath.expmap0(x, c=self.c), c=self.c)) , self.c
...
class FromPoincare(nn.Module):
    ...
    def forward(self, x):
        if self.train_x:
            xp = pmath.project(pmath.expmap0(self.xp, c=self.c), c=self.c)
            return pmath.logmap(xp, x, c=self.c),self.c
        return pmath.logmap0(x, c=self.c),self.c
